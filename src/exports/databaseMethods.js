...
const {
  BONUS_WITHIN_TIME_LIMIT,
  BONUS_WITHIN_TIME_LIMIT_MULTIPLIER,
  FIRST_BLOOD_ADDITIONAL_POINTS,
} = require("./CHALLENGE_CONSTANTS.json");
...
const {
  ...
  scoreboardSchema,
  longAnswerSchema,
} = require("./challengeSchemas.js");
const {
  DATABASE_NAME_CHALLENGE,
  DATABASE_NAME_SERVER,
...
const FlagsObtained = challengeConnection.model(
  "FlagsObtained",
  flagsObtainedSchema,
);
...
const ServerBotAdmin = serverConnection.model(
  "ServerBotAdmin",
...
async function createFlag(
  ...
  try {
    ...
    const existingFlag = await Flag.find({
    ...
    if (existingFlag.length != 0) {
      ...
      const newFlag = new Flag({
        ...
        value: value,
        isLongAns: false,
        submissionOpenDate: submissionOpenDateProc,
        dateCreated: dateCreated,
      });
      await newFlag.save();
...
async function joinChallenge(challengeID, discordID) {
  const dateCreated = new Date();
  const existingPlayer = await Scoreboard.find({
    challengeID: challengeID,
    playerID: discordID,
  });
  ...
  const isOver =
    new Date() > new Date(challenge.startDate.getTime() + challenge.duration);
  ...
  if (existingPlayer.length != 0) {
    ...
    const scoreboardEntry = new Scoreboard({
      challengeID: challengeID,
      playerID: discordID,
      scoreValue: 0,
      dateCreated: dateCreated,
...
async function createChallenge(
  ...
  try {
    ...
    const challenge = new Challenge({
      ...
      isHiddenID: true,
      longAnsChannelID: longAnsChannelID,
      isOpen: false,
      isTargeted: false,
      isBonusTimeLimit: false,
      isFirstBlood: false,
      dateCreated: dateCreated,
      logChannelID: DEFAULT_CHALLENGE_LOG_ID,
      puzzleMakerID: "123456789012456789",
    });
...
async function submitFlag(
  ...
  const scoreboardPlayer = await Scoreboard.find({
    challengeID: challengeID,
    playerID: discordID,
  });
  if (scoreboardPlayer.length == 0) {
    ...
    if (existingChallenge.length == 0) {
      throw new Error(
        "Challenge does not exist: Is your challenge ID correct?",
      );
    } else {
      throw new Error(
        `You have not joined the challenge. Use ${inlineCode("/challenge join " + challengeID)} to join the challenge first.`,
      );
    }
  }
  ...
  const existingSubmission = await FlagsObtained.find({
    challengeID: challengeID,
    playerID: discordID,
    flag: flag.flag,
  });
  ...
  if (hasAdditionalInput) {
    ...
    const longAnsSubmission = new LongAnswer({
      challengeID: challengeID,
      playerID: discordID,
      flagID: flag._id,
      contents: additionalInput,
    });
  ...
  if (existingSubmission.length == 0) {
    const flagSubmission = new FlagsObtained({
      challengeID: challengeID,
      playerID: discordID,
      flag: flagString,
      dateCreated: dateCreated,
  ...
  const withinHours =
    (flag.submissionOpenDate - dateNow) / (1000 * 60 * 60) <=
    BONUS_WITHIN_TIME_LIMIT; // ms to hours -- checks against hour cap for bonus
  var flagValueProc =
    challenge.isBonusTimeLimit == true && withinHours
      ? flagValue * BONUS_WITHIN_TIME_LIMIT_MULTIPLIER // if within time limit, multiply score
      : flagValue;

  // Adds first blood bonus if applicable
  const submissions = await FlagsObtained.find({
    challengeID: challengeID,
    flag: flag.flag,
  });
  const isFirstSubmission = submissions.length == 1; // 1 for this flag submission
  flagValueProc =
    challenge.isFirstBlood == true && isFirstSubmission == true
      ? flagValueProc + FIRST_BLOOD_ADDITIONAL_POINTS
      : flagValueProc;

  const scoreboardUpdateRes = await Scoreboard.findOneAndUpdate(
    { challengeID: challengeID, playerID: discordID },
    { $inc: { scoreValue: flagValueProc } },
  );

  if (scoreboardUpdateRes.length == 0) {
    // default error handling
    throw new Error(
      "Scoreboard update unsuccessful: No scoreboard entry found.",
    );
  }
...
async function findChallenge(param) {
  ...
  try {
    ...
    const multipleChallengesErrMsg =
      "Unexpected behaviour: Multiple challenges found (findChallenge).";
...
async function findLongAns(longAnsID) {
  try {
    ...
    if (longAnsById.length == 0) {
      ...
      throw new Error(
        "Unexpected behaviour: Multiple long answers found (findLongAns).",
      );
    } else {
      return longAnsById;
    }
...
async function findScore(challengeID, discordID) {
  const res = await Scoreboard.find({
    challengeID: challengeID.toString(),
    playerID: discordID,
  });
  if (res.length == 0) {
    throw new Error(
      "Scoreboard entry does not exist. Have you joined the challenge?",
    );
  } else {
...
async function addServerBotAdmin(serverID, roleID) {
  ...
  if (serverBotAdminEntry.length === 0) {
    ...
    const serverBotAdminSubmission = new ServerBotAdmin({
      serverID: serverID,
      roleID: roleID,
    });
    ...
    const serverBotAdminUpdateRes = await ServerBotAdmin.findOneAndUpdate(
      { serverID: serverID },
      { $set: { roleID: roleID } },
      { new: true },
    );
