...
2) [Challenges](commands/challenge) \[WIP\]
...
# config.json
You need to create a `config.json` file in the root directory. It is very important to keep this file secret, especially your bot token; don't commit the file to GitHub! Feel free to change the cooldowns (CD) to suit your needs, but note that the `ADMIN_ID` user is immune to the cooldowns. With the current code, here are the variables you will need to define:
